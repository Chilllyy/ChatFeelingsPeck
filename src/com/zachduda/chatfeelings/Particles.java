...
public class Particles {
    ...
    private static final int particleVersion = Supports.getParticleVersion();
		...
		static void show(Player p, String label) {
            ...
            try {
                ...
                if(Main.debug) {
                    Main.debug("Particle Error Thrown:" + err.getMessage());
                    err.printStackTrace();
                }
            }
        }
    ...
    private static void wbParticle(Player p) {
        World world = p.getLocation().getWorld();
        assert world != null;
        if(Supports.getMcMajorVersion() >= 1 && Supports.getMcMinorVersion() >= 17) {
            world.spawnParticle(Particle.GLOW_SQUID_INK, p.getLocation().add(0, 1.3, 0), 20, 2.0D, 1.0D, 2.0D, 0.001D);
        } else {
